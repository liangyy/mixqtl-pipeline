...
# the R function for permutation used in the old repo
permute_x_ = function(x, perm_mat) {
  if(ncol(perm_mat) != ncol(x) | nrow(perm_mat) != nrow(x)) {
    message('Unmatched ncol between x and perm_mat')
    return(NA)
  }
  for(i in 1 : ncol(perm_mat)) {
    x[, i] = x[perm_mat[, i], i]
  }
  x
}
...
perm_mat = get_permutations(ncol(geno1), nrow(df))
geno1 = permute_x_(geno1, perm_mat)
geno2 = permute_x_(geno2, perm_mat)
