...
p = my_qqplot(df$pval, df$method) 
p = p + geom_abline(intercept = 0, slope = 1)
p = p + scale_color_manual(values = cbPalette) + theme(legend.position = c(0.3, 0.5), legend.title = element_blank()) + th + theme(aspect.ratio = 1) + ylab(expression(-log[10](p[observed]))) + xlab(expression(-log[10](p[expected])))
p$layers[[2]]$aes_params$size = 0.5
p
...
for(m in c('ascQTL', 'trcQTL', 'mixQTL')) {
  ...
  p$layers <- c(geom_abline(slope = 1, intercept = 0, color = 'lightgray'), p$layers)
  p = p + scale_color_manual(values = cbPalette)
  p = p + theme(legend.position = c(0.3, 0.5), legend.title = element_blank()) + th + theme(aspect.ratio = 1) + ylab(expression(-log[10](p[observed]))) + xlab(expression(-log[10](p[expected])))
