...
# df_both_complete = df_both_complete %>% filter(gene %in% expression_level$gene[expression_level$n_good_asc > 30 / 3 * 10]) 
```
...
df_subsample_with_consensus = df_subsample_complete %>% filter(variable_prob > 0) %>% mutate(is_consensus = paste(variant_id, gene) %in% paste(consensus_eqtl$variant_id, consensus_eqtl$gene))
...
p = temp %>% ggplot() + geom_violin(aes(x = as.factor(fraction), y = fraction_of_detection, fill = method), alpha = .5, position = position_dodge(1), width = 1.5) + 
  # geom_boxplot(aes(x = as.factor(fraction), y = fraction_of_detection, fill = method), alpha = 1, width = .2, position = position_dodge(1)) + 
  scale_fill_manual(values = cbPalette) + xlab('Subsampling fraction') + ylab('Power (detection rate)') + theme(legend.position = c(0.8, 0.2), legend.title = element_blank()) + geom_jitter(aes(x = as.factor(fraction), y = fraction_of_detection, fill = method), position = position_dodge(1), size = 0.7, alpha = 0.5) # + ggtitle('Consensus variants \n (PIP > 0.5 in both) \n being detected')
...
ggsave('../output/gtex-v8-pipeline-eqtl-finemapping-concensus-snp-with-gene-qc.pdf', p, height = 4, width = 4)
```
