...
then
  bash $scriptdir/for_gencode_gene_annotation.sh
fi

# subset vcf file to European individuals
if [[ ! -f EUR.chr22.phase3_shapeit2_mvncall_integrated_v5a.20130502.genotypes.vcf.gz ]]
then
  bcftools view -S EUR.sample_list_in_phase3.txt ALL.chr22.phase3_shapeit2_mvncall_integrated_v5a.20130502.genotypes.vcf.gz | gzip > EUR.chr22.phase3_shapeit2_mvncall_integrated_v5a.20130502.genotypes.vcf.gz
fi

# sample some genes from chromosome 22
if [[ ! -f selected_genes.txt ]]
then
  Rscript $scriptdir/sampling_genes.R <(zcat formatted-gene-model.gencode.v32lift37.basic.annotation.txt.gz|grep chr22) 1000000 selected_genes.txt
fi 
